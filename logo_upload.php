...
*/
        $uploaddir = "Kunden/" . $result_id . "/" . "logo/" . "/";
        ...
        $logo = $uploaddir ; //. dateiname_bereinigen(basename($_FILES['speisekarte']['name']));
        $dateiname = "logo.png";
        if ( $_FILES['logo']['name'] <> '' ) 
        {
          // if (move_uploaded_file($_FILES['speisekarte']['tmp_name'], $speisekarte)) {
            if (move_uploaded_file($_FILES['logo']['tmp_name'], $logo . $dateiname)) {
              $bilddateivorschlag = "1";
              $mysqli->query("UPDATE users SET profilbild='1' WHERE id='$result_id'");

            echo "Bilddatei ist valide und wurde erfolgreich angelegt. \n";
          } else {
